...
python run_pipeline_full.py --dataset ${DATASET} --model meta-sgd --lr 0.1 \
    --meta_method reptile --meta_strategy sequential --num_lookahead_tasks 3 --meta_lr 0.2 \
    --meta_adapt_lr 0.005 --meta_adapt_steps 10 --num_meta_examples 50 \
    --adapt_lr 0.005 --num_adapt_steps 10 \
    --savecheck task
EOF
            ...
            sbatch <<EOF
#!/bin/bash
#SBATCH --job-name=${DATASET}_meta_sgd_parallel
#SBATCH --output=logs/${DATASET}/meta_sgd_parallel_%j.out
#SBATCH --error=logs/${DATASET}/meta_sgd_parallel_%j.err
#SBATCH --time=01:00:00
#SBATCH --partition=short,burst
#SBATCH --account=zgroup
#SBATCH --gres=gpu:A6000:1
...
source .mammoth/bin/activate
python run_pipeline_full.py --dataset ${DATASET} --model meta-sgd --lr 0.1 \
    --meta_method no_meta --meta_strategy parallel --num_lookahead_tasks 3 --meta_lr 0.2 \
    --meta_adapt_lr 0.005 --meta_adapt_steps 10 --num_meta_examples 50 \
    --adapt_lr 0.005 --num_adapt_steps 10 \
    --savecheck task
EOF
